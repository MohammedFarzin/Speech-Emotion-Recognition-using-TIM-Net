...
class TIMNET_Model(Common_Model):
    ...
    def create_model(self):
        ...
        self.multi_decision = TIMNET(nb_filters=self.args.filter_size, kernel_size=self.args.kernel_size, nb_stack=self.args.stack_size, dilations=self.args.dilation_size,
                                    activation=self.args.activation, dropout_rate=self.args.dropout, return_sequence=True, name='TIMNET')(self.inputs)
    ...
    def train(self, x, y):
        ...
        for train, test in kfold.split(x, y):
            self.create_model()
            y[train] = smooth_labels(copy.deepcopy(y[train]), 0.1)
