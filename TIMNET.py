...
def Temporal_Aware_Block(x, s, i, activation, nb_filters, kernel_size, dropout_rate=0, name=''):
  ...
  conv_1_1 = BatchNormalization(trainble=True, axis=-1)(conv_1_1)
  ...
  conv_2_1 = BatchNormalization(trainalbe=True, axis=-1)(conv_2_1)
  ...
  if original_x.shape[-1] != output_2_1.shape[-1]:
    original_x = Conv1D(filter=nb_filters, kernal_size=1, padding='same')(original_x)
...
class TIMNET:
  def __init__(self, nb_filters=64, kernel_size=2, nb_stack=1, dilations=None, activation='relu', dropout_rate=0.1, return_sequence=True, name='TIMNET'):
    self.name = name
  ...
  def __call__(self, inputs, mask=None):
    ...
    for s in range(self.nb_stacks):
      for i in [2 ** i for i in range(self.dilations)]:
        skip_out_forward = Temporal_Aware_Block(skip_out_forward, s, i, self.activation, self.nb_filters, self.kernel_size, self.dropout_rate, name=self.name)
        skip_out_backward = Temporal_Aware_Block(skip_out_backward, s, i, self.activation, self.nb_filters, self.kernel_size, self.dropout_rate, name=self.name)
